    ...
    <div style="padding: 0 10%">
        <div class="bg-white p-20" style="text-align: left; min-height: 80vh">
            ...
            <div class="w-100 flex-r flex-jc-r flex-ai-c m-t-20 end-sum">
                ...
                <div><el-button @click="submit" type="danger" size="large" style="width: 100px" :disabled="shopnum == 0">结算</el-button></div>
            </div>
...
export default {
    ...
    // },
    mounted() {
        this.shopbooks = localStorage.getItem('shopbooks') ? JSON.parse(localStorage.getItem('shopbooks')) : []
        console.log(this.shopbooks);
        this.shopnum = this.shopbooks.length
    ...
    methods: {
        ...
        submit() {
            alert('购买成功')
            let arr = localStorage.getItem('orderList') ? JSON.parse(localStorage.getItem('orderList')) : []
            this.shopbooks.map((n) => {
                arr.push(n)
            })
